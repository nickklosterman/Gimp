...
;input-depthmap and original image
;process-query for amount of displacement
;I thought to ask for amount of displacement as well as if they wanted to rescale teh displacement map, but they really go hand in hand. I think that for a given dispacement map and amount of displacement that if you rescale the map to 128-0 and double the amount of displacement that you'll get the exact same thing.

;copy original image 2x
;set G/B channels to off
;set blending mode to screen
;apply depth map
;copy original image
;set R channel off
;apply depth map
;turn visibility of depth map and original image off?
;output-

;need to create two new layers
;

(define (script-fu-Anaglyph-from-Displacement-Map
				theImage
				drawable
...
; Variable init
  (let* (
	...
	(i 0)
	(RGBImage)
	(displacement_amountX 10)
	(displacement_amountY 0)
	(num_layers 0)
	(layer_ids)
	(current_layer)
	(theDisplay)
    )
(gimp-selection-all SomeImgOrLayer)
(gimp-layer-set-mode SomeLayer SCREEN-MODE)
...
; Add new layer
	(gimp-layer-copy drawable FALSE)
	(gimp-layer-copy drawable FALSE)
